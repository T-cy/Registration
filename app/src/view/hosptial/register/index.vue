<template>
  <div class="register" >
    ...
    <div class="department">
      <div class="left">
        <ul>
          <li :class="{active:activeIndex==index}" v-for="(item,index) in HosptialDetail.hosptialDepatement" :key="item.depcode" @click="changeIndex(index)">
            {{ item.depname }}
          </li>
        </ul>
      </div>
      <div class="right">
        <div class="showDepartment" v-for="(item) in HosptialDetail.hosptialDepatement" :key="item.depcode">
          <h1 class="Nametitle" >{{ item.depname }}</h1>
          <!-- 每一个科室下面的小科室 -->
          <ul>
            <li  v-for="(item1) in item.children" :key="item1.depcode">
              {{ item1.depname }}
            </li>
          </ul>
        </div>
...
let changeIndex=(index:number)=>{
  ...
  let Nametitle=document.querySelectorAll('.Nametitle')
  Nametitle[index].scrollIntoView({
    behavior:'smooth'
  })
}
